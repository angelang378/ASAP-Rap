...
def generate_lr_graphs(err_tr, print_list, model_num):
    plt.figure()
    ...
    plt.title("Model {} Training Loss Over Epochs".format(model_num))
...
def test_learning_rate(epochs=50, freq=20):
    learning_rates = [0.001, 0.003, 0.005, 0.007, 0.009]
    errs_tr = []
    errs_te = []
    # checkat = [1, 21, 41, 61, 81, 101, 121, 141, 161, 181, 200]
    checkat = [1, 11, 21, 31, 41, 50]
    for i in learning_rates:
        err_tr, err_te = train_model("data/vivian.csv", "data/william.csv",
                                     "Vivian", "William", epochs, i, freq)
        errs_tr.append(err_tr)
        errs_te.append(err_te)

    print(errs_te)
    generate_lr_graphs(errs_tr, checkat)
...
def generate_epoch_graphs(err_tr, err_te, print_list):
    ...
    plt.title("Model 2 Testing vs Training Loss Over Epochs")
...
def test_epochs(learning_rate=0.001):
    ...
    te_errs = []
    for i in epochs:
        tr_err, te_err = train_model("data/4a6u0ZVG0FWYAJHVggnHAh.csv",
    ...
    print(te_errs)
    generate_epoch_graphs(tr_errs, te_errs, epochs)


def main():
    # _, te_err = train_model("data/4a6u0ZVG0FWYAJHVggnHAh.csv",
    #                         "data/1ukuCLLRLSXE7WYWlbEq2n.csv", "Vivian",
    #                         "William")
    predict('data/beatles.csv', "trained_models/trained_model16.pt")
